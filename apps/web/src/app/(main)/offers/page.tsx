...
import {
  FileText, ChevronDown, Clock, MapPin,
  ExternalLink, Undo2, ArrowRightLeft, Loader2,
} from 'lucide-react';
...
export default function MyOffersPage() {
  ...
  const myOffers = useMemo(() => {
    let result = activeTab === 'all' ? [...allOffers] : allOffers.filter((o) => o.status === activeTab);
  ...
  const tabCounts = useMemo(() => ({
    ...
    counter_offered: allOffers.filter((o) => o.status === 'counter_offered').length,
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      <div className="mb-8">
        <h1 className="text-h1 font-bold text-neutral-900 dark:text-dark-textPrimary">Tekliflerim</h1>
        <p className="mt-2 text-body-lg text-neutral-500">Gönderdiğin teklifleri takip et ve yönet.</p>
      </div>
          ...
          return (
            <button
              ...
              className={`shrink-0 px-4 py-2.5 rounded-lg text-body-md font-medium transition-colors ${
                activeTab === tab.value ? 'bg-primary text-white' : 'text-neutral-500 hover:bg-neutral-100 dark:hover:bg-dark-surfaceRaised'
              }`}
            >
              {tab.label}
              {count > 0 && <span className={`ml-1.5 text-body-sm ${activeTab === tab.value ? 'text-white/70' : 'text-neutral-400'}`}>({count})</span>}
            </button>
          );
        })}
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-body-md text-neutral-500"><strong className="text-neutral-700 dark:text-dark-textPrimary">{myOffers.length}</strong> teklif</p>
        <div className="relative">
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="h-10 pl-4 pr-9 rounded-lg border border-neutral-200 dark:border-dark-border bg-white dark:bg-dark-surface text-body-md text-neutral-700 dark:text-dark-textPrimary appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/20">
      ...
      {myOffers.length > 0 ? (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 bg-primary-lighter dark:bg-primary/20 text-primary dark:text-blue-300 text-body-sm font-medium rounded-sm">{offer.listingCategory}</span>
                    <OfferStatusBadge status={offer.status} />
                  </div>
                  <Link href={`/listing/${offer.listingId}`} className="text-h4 font-semibold text-neutral-900 dark:text-dark-textPrimary hover:text-accent transition-colors line-clamp-2 mb-2 block">
                    {offer.listingTitle}
                  </Link>
                  <div className="flex flex-wrap items-center gap-3 text-body-sm text-neutral-400">
                    <span className="flex items-center gap-1"><MapPin size={13} /> {offer.listingCity}</span>
                    <span className="flex items-center gap-1"><Clock size={13} /> {timeAgo(offer.createdAt)}</span>
                  </div>
                </div>
              ...
              {offer.status === 'counter_offered' && offer.counterPrice && (
                <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-500/10 rounded-lg border border-blue-100 dark:border-blue-500/20">
                  ...
                  <p className="text-body-md font-bold text-blue-700 dark:text-blue-300">
                    {formatCurrency(offer.counterPrice)}{offer.counterDays ? ` · ${offer.counterDays} gün teslimat` : ''}
                  </p>
                  {offer.counterNote && <p className="text-body-sm text-blue-600/80 dark:text-blue-400/80 mt-1 line-clamp-2">{offer.counterNote}</p>}
                </div>
              )}
              ...
              {offer.status === 'rejected' && offer.rejectedReason && (
                <div className="mt-4 p-3 bg-red-50 dark:bg-red-500/10 rounded-lg border border-red-100 dark:border-red-500/20">
                  <p className="text-body-sm text-red-600 dark:text-red-400"><strong>Red sebebi:</strong> {offer.rejectedReason}</p>
                </div>
              )}

              <div className="mt-4 pt-4 border-t border-neutral-100 dark:border-dark-border flex items-center gap-3">
                <Link href={`/offers/${offer.id}`} className="h-9 px-4 rounded-lg border border-neutral-200 dark:border-dark-border text-body-sm font-medium text-neutral-600 dark:text-dark-textSecondary hover:bg-neutral-50 dark:hover:bg-dark-surfaceRaised transition-colors flex items-center gap-2">
                ...
                {offer.status === 'counter_offered' && (
                  <Link href={`/offers/${offer.id}`} className="h-9 px-4 rounded-lg bg-accent text-white text-body-sm font-semibold hover:bg-accent-600 transition-colors flex items-center gap-2">
        ...
        <div className="text-center py-20">
          ...
          <h3 className="text-h3 font-semibold text-neutral-700 dark:text-dark-textPrimary mb-2">
            {activeTab === 'all' ? 'Henüz teklifin yok' : 'Bu kategoride teklifin yok'}
          </h3>
          <p className="text-body-lg text-neutral-500 mb-6">İlanları keşfet ve ilk teklifini ver!</p>
          <Link href="/explore" className="inline-flex items-center gap-2 h-11 px-6 bg-accent text-white text-body-md font-semibold rounded-lg hover:bg-accent-600 transition-colors">
            İlanları Keşfet
          </Link>
        </div>
      )}
