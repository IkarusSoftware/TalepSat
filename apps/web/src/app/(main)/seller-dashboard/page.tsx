...
import {
  ...
  ArrowRight, ExternalLink, Clock, ArrowRightLeft, FileText, Loader2,
} from 'lucide-react';
...
export default function SellerDashboardPage() {
  ...
  const recentOffers = useMemo(() => {
    return [...offers].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, 5);
  }, [offers]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      <div className="mb-8">
        <h1 className="text-h1 font-bold text-neutral-900 dark:text-dark-textPrimary">Satıcı Paneli</h1>
        <p className="mt-2 text-body-lg text-neutral-500">Teklif performansını takip et, kazancını gör.</p>
      </div>
      ...
      {offers.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="text-h3 font-semibold text-neutral-700 dark:text-dark-textPrimary mb-2">Henüz teklif vermedin</h3>
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-h3 font-semibold text-neutral-900 dark:text-dark-textPrimary">Son Teklifler</h2>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <Link href={`/offers/${offer.id}`} className="text-body-md font-semibold text-neutral-900 dark:text-dark-textPrimary hover:text-accent transition-colors line-clamp-1 block">
                        {offer.listingTitle}
                      </Link>
          ...
          <div className="lg:col-span-2 space-y-5">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.4 }} className="bg-white dark:bg-dark-surface rounded-xl border border-neutral-200/50 dark:border-dark-border/80 p-6">
              <h3 className="text-h4 font-semibold text-neutral-900 dark:text-dark-textPrimary mb-5 flex items-center gap-2">
                <TrendingUp size={18} className="text-accent" /> Performans Özeti
              </h3>
              <div className="space-y-4">
                <div>
                  ...
                  <div className="w-full h-2.5 bg-neutral-100 dark:bg-dark-surfaceRaised rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${stats.acceptRate}%` }} transition={{ duration: 0.8, delay: 0.6, ease: 'easeOut' }} className="h-full bg-success rounded-full" />
                  </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.5 }} className="bg-white dark:bg-dark-surface rounded-xl border border-neutral-200/50 dark:border-dark-border/80 p-5">
